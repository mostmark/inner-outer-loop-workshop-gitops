...
|---|---|
| After the cluster was hibernated or restarted, new Istio `Gateway`s (the per-user ingress gateways of the Service Mesh module) do not route traffic | The gateway pods keep stale configuration from before the hibernation. Restart them: `oc rollout restart deployment -l istio=ingressgateway-<user> -n cn-project-<user>` for affected users, or restart istiod first: `oc rollout restart deployment/istiod -n istio-system`, then the gateways. |
