...
| K17 | The `WORKSHOP_USER_PASSWORD` used on the test cluster is the old workshop's well-known default password, which the imported upstream code hard-coded. It therefore appears in the code repository's history (the verbatim import commit and the diff of the commit that removed it), although no current file and no chart contains it. | Anyone who reads the public history learns that default, not a secret of this setup. | Use a different, event-specific password for real events (`WORKSHOP_USER_PASSWORD` is only stored in the cluster). |
